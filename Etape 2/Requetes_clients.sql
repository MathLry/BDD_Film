...
-- requete n°4
SELECT film.numero_film, seance.date_de_la_seance AS date, 
	   seance.horaire AS horaire
FROM seance
...
-- requete n°7
SELECT acteur.nom_acteur AS Acteur, 
	   realisateur.prenom_realisateur || ' ' || realisateur.nom_realisateur AS Réalisateur
FROM realisateur
...
-- requete n°8
SELECT prenom_acteur || ' ' || nom_acteur AS Acteur, 
	   COUNT(role.numero_acteur) AS 'Nombre de rôle réalisé'
FROM acteur
...
SELECT prenom_acteur || ' ' || nom_acteur AS Acteur, 
	   COUNT(seance.numero_seance) AS 'Nombre de séances'
FROM acteur
INNER JOIN role ON role.numero_acteur = acteur.numero_acteur
